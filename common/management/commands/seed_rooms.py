...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options['delete']:
            ...
            number = len(rooms)
            rooms.delete()
            self.stdout.write(self.style.SUCCESS(f'{number} rooms deleted!'))
        else:
            ...
            seeder.add_entity(
                Room, number, {
                    ...
                    'latitude': lambda x: round(random.uniform(-90, 90), 6), 
                    'longitude': lambda x: round(random.uniform(-180, 180), 6),
                }
            )
